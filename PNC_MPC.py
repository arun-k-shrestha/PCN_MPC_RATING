...
def find_valid_path_with_mpc(G, sender, receiver, send_amount):
    valid_path = None
    visited_edges = set()
        ...
        if knows_preimage[receiver]:
            ...
            if not G.nodes[receiver]['honest']:
                ...
                for u, v in locked_edges:
                    G[u][v]['balance'] += amount # this amount might be negative, becuase the nodes might have lied during the MPC check   
                return False
...
for i in range(1000):
    ...
    mpc_valid_path= find_valid_path_with_mpc(G, node1, node2, amount)
